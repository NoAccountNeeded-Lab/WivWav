 ...
 *
 * Usage:
 *   pnpm tsx apps/api/src/jobs/canonicalize-backfill.ts --report
 *   pnpm tsx apps/api/src/jobs/canonicalize-backfill.ts --apply
 *
 * Always run --report first to review the scope of changes before --apply.
 *
 * Deployment procedure:
 * 1. Deploy the schema migration (adds engine column).
 * 2. Run: pnpm tsx apps/api/src/jobs/canonicalize-backfill.ts --report
 *    Review output — confirm affected counts and sample values look correct.
 * 3. Run: pnpm tsx apps/api/src/jobs/canonicalize-backfill.ts --apply
