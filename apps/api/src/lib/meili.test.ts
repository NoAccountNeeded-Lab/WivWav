...
describe('getMeiliClient', () => {
  ...
  it('defaults host to http://localhost:7700 when MEILISEARCH_HOST is unset', async () => {
    vi.stubEnv('MEILISEARCH_API_KEY', 'test-key')
    const orig = process.env['MEILISEARCH_HOST']
    ...
    try {
      ...
      if (orig !== undefined) process.env['MEILISEARCH_HOST'] = orig
    }
  })

  it('omits apiKey from config when MEILISEARCH_API_KEY is unset', async () => {
    vi.stubEnv('MEILISEARCH_HOST', 'http://localhost:7700')
    const orig = process.env['MEILISEARCH_API_KEY']
    ...
    try {
      ...
      if (orig !== undefined) process.env['MEILISEARCH_API_KEY'] = orig
    }
  })

  it('uses MEILISEARCH_HOST when set', async () => {
    vi.stubEnv('MEILISEARCH_HOST', 'http://meili.internal:7700')
    vi.stubEnv('MEILISEARCH_API_KEY', 'prod-key')
    const { getMeiliClient } = await import('./meili.js')
    const client = getMeiliClient()
    expect((client as unknown as { config: { host: string } }).config.host).toBe(
