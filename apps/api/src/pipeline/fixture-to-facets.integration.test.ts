 ...
 * (packages/scraper-sources/src/sources/fixtures/contracts/).
 *
 * Unlike the mocked unit tests it complements (apps/api's facet/listings
 * tests mock Meilisearch distributions; apps/api's job tests mock
 ...
 *   - deduplication:         runDeduplicateJob (./jobs/deduplicate.ts)
 *   - publication resolution: runListingResolveJob (./jobs/listing-resolve.ts)
 *   - full search sync:      runMeilisearchSyncJob (./jobs/meilisearch-sync.ts)
 *   - facets/listings API:   apps/api's real ListingFacetsService / ListingSearchService /
 *                            MeilisearchService, called directly (short of the actual
 *                            Fastify route — see below).
 *
 * This suite used to live in apps/scraper and import these apps/api service
 * modules by a deliberate cross-app relative-path exception (no package
 * dependency declared) because it needed both the scraper-ingestion and
 * API-facet halves of the pipeline in one place. #970 relocated the whole
 * suite into apps/api alongside the services it already exercises, so that
 * exception no longer applies — every import below is a normal same-package
 * relative import. It still stops at the service layer
 ...
 * (covered by apps/api/src/routes/listings.test.ts).
 *
 * Local run:   pnpm --filter @wivwav/api exec vitest run src/pipeline/fixture-to-facets.integration.test.ts
 ...
 * the only other apps/api integration spec that doesn't hit a live
 * third-party website — runs via the root `pnpm test:integration` script,
 * which now runs apps/api's whole test:integration target (turbo
 * `--filter=@wivwav/api`) rather than a separate offline-only subset.
 *
...
import {
  ...
  type SearchParams,
} from '../services/listing-search.js'
import {
  ListingFacetsService,
  type FacetsParams,
} from '../services/listing-facets.js'
import { MeilisearchService } from '../services/search/meilisearch-service.js'
import { MemoryCacheService } from '../services/cache/memory-cache-service.js'
...
// Default to the same real, disposable Meilisearch instance runMeilisearchSyncJob
// and the facets/listings services below both read via MEILISEARCH_HOST/MEILISEARCH_API_KEY
// (apps/api/src/lib/meili.ts) so they operate on the exact same index.
...
describe('fixture-to-facets pipeline contract (#640)', () => {
  ...
  async function buildFixtureDerivedRows(session: BrowserSession): Promise<{
    ...
    try {
      ...
      const blvdDetail: DetailResult = {
        ...blvdDetailFields,
        // Neither fixture source exposes a listing/update timestamp on the
        // detail page — always null for these adapters, same as production.
        sourceListedAt: null,
      ...
      const mwDescriptionEvidence = mwRawDetail.descriptionFound
      ...
      const mwDetail: DetailResult = {
        ...
        evidence: {
          ...
          description: mwDescriptionEvidence,
          accessibilityClaims: mwDescriptionEvidence,
  ...
  // NOTE: packages/scraper-sources/src/sources/fixtures/contracts/blvd-list-v2-recrawl.html
