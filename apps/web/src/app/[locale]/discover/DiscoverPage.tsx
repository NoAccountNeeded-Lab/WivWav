...
function intakeFiltersToParams(intake: IntakeFilters): Record<string, string> {
  ...
  if (intake.wavFeatures != null && intake.wavFeatures.length > 0) params['wavFeatures'] = intake.wavFeatures.join(',')
...
function fmtLabel(key: string, value: string): string {
  ...
  if (key === 'wavFeatures') return value.split(',').map(f => f.replace(/_/g, ' ')).join(', ')
