...
2. **Read issue details** — before planning or reading source files, prefer the local context generated by `run-sprint`:
   ```bash
   sed -n '1,220p' .agents/worker-context.md
   sed -n '1,260p' .agents/issue-context.md
   ```
   Fetch GitHub only if the local artifacts are missing or you need live issue updates:
   ```bash
   gh issue view {N} --json number,title,body,labels
   ```
   Use `.agents/issue-context.md` as the source of truth for acceptance criteria when present. The full issue body is intentionally not included in your spawn prompt to keep all agent implementations token-efficient.
...
9. **Usage report** — update `.agents/usage-report.md` with the provider, model, token counts, cache token counts, and tool-call count exposed by your runtime. If token counts are unavailable, record `unavailable` and still name the model used.
