...
2. Follow CLAUDE.md workflow exactly — no exceptions:
   ...
   - Once CI passes: run `gh pr checks {PR#}` and confirm both `ci` and `gates` are green, check the `- [x] CI passes` and `- [x] Code review findings are resolved or tracked` boxes in the PR body, add the `status:needs-review` label (`gh pr edit {PR#} --add-label "status:needs-review"`), mark the PR ready (`gh pr ready {PR#}`), then merge (`gh pr merge {PR#} --rebase --delete-branch`) and run `git pull origin main && pnpm install`
