...
import { SuperiorVanAdapter } from './superior-van.js'

// Integration tests — hit the real superiorvan.com via Playwright.
// Run: pnpm --filter @wivwav/scraper test:integration

describe('SuperiorVanAdapter', () => {
  it('checkStructure returns a consistent hash', async () => {
    const adapter = new SuperiorVanAdapter(null, { maxPages: 1 })
  ...
  it('scrapes at least 5 listings from page 1', async () => {
    const adapter = new SuperiorVanAdapter(null, { maxPages: 1 })
  ...
  it('each listing has required vehicle fields', async () => {
    ...
    for (const listing of listings.slice(0, 5)) {
      expect(listing.sourceId).toBe('superior-van')
      expect(listing.sourceUrl).toContain('superiorvan.com/inventory/')
      ...
      expect(listing.dealer.name).toBe('Superior Van & Mobility')
    }
  }, 60_000)

  it('paginates across multiple pages returning more unique listings', async () => {
    const adapter = new SuperiorVanAdapter(null, { maxPages: 2 })
    const { listings } = await adapter.scrape()
