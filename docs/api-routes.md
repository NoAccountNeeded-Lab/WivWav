...
| POST   | /internal/ops/problem-ack      | Acknowledge or unacknowledge a persisted aggregated problem fingerprint. Body: `{ fingerprint, acknowledged, acknowledgedBy? }`. Acknowledging sets `acknowledgedAt` and `acknowledgedBy`; unacknowledging clears both. The row must already exist in `ops_problem_state`, which is populated by aggregation passes and retains resolved/flapping problem history. Uses the same fail-closed bearer guard as private ops routes. |
