...
| GET    | /admin/grafana/alerts          | Current Grafana alert-instance state against the rules in `docker/grafana/provisioning/alerting/wivwav-alert-rules.yaml` (issue #890, federated into the problem aggregate). Response: `{ data: { alerts: GrafanaAlertInstance[], unavailable: boolean } }`. Never fails: an unreachable/erroring Grafana or a non-2xx response returns `unavailable: true` with an empty `alerts` list rather than a non-2xx HTTP status. Requires `GRAFANA_URL` env var (default `http://localhost:3003`); `GRAFANA_API_TOKEN` is optional (local dev's Grafana allows anonymous admin access). |
| GET    | /admin/sentry/issues           | Recent unresolved Sentry issue summaries for the configured project (issue #890, federated into the problem aggregate). Response: `{ data: { issues: SentryIssueSummary[], unavailable: boolean } }`. Never fails: missing credentials, an unreachable Sentry API, or a non-2xx response all return `unavailable: true` with an empty `issues` list. Requires `SENTRY_ISSUES_AUTH_TOKEN`/`SENTRY_ISSUES_ORG`/`SENTRY_ISSUES_PROJECT` env vars — a read-only token distinct from apps/web's build-time `SENTRY_AUTH_TOKEN`/`SENTRY_ORG`/`SENTRY_PROJECT` (source-map upload scope only). |
